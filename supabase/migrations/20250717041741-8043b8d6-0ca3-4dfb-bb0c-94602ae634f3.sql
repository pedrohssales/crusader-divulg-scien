...
USING (
  ...
  (auth.uid() IS NOT NULL AND status IN ('draft', 'pending', 'rejected', 'returned', 'retained'))
);
...
-- Admins can still update any publication
-- This policy already exists: "Admins can update any publication"